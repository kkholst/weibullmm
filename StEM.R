...
##   dyn.unload(mylib)
## try(dyn.load(mylib))  
if (!require(coda)) print("Woops no coda")
...
print.StEM <- function(x,burnin=nrow(x$theta)/2,...) {
  with(x, {cat(" Model: '",model,"' with ", modelpar$nlatent, " latent variable",sep="");
              if(modelpar$nlatent>1) cat("s",sep=""); cat(".\n", sep="") })
  with(x, cat(nrow(theta), " StEM iterations with an E-step based on ", control$nsim, " MCMC samples.\n Average acceptance rate: ", formatC(mean(mc$accept)/control$nsim),"\n", sep=""))
  cat("Estimated mean parameter (burnin=",burnin,"):\n",sep="");
  res <- coef(x,burnin,...)
  print(res)
  cat("\n")
