...
## tol1 <- tol2 <- 1e-9; 
## for (i in 1:niter) {
##   H <- hessian.weibull(theta,all=TRUE,T,status,X)
##   S <- colSums(attributes(H)$score)
##   theta <- theta - solve(H)%*%S
##   thetas <- rbind(thetas,as.vector(theta))
##   logL <- c(logL, attributes(H)$logL)
##   if (i>1)
##   if (sum(abs(S^2))<tol1 & abs(logL[i]-logL[i-1])<tol2) break;
## }
## I <- -H
## coefs <- cbind(theta,diag(solve(I))^0.5); colnames(coefs) <- c("Estimate","Std.Err")
## coefs
## tail(logL,1)
## S
## J <- t(attributes(H)$score)%*%(attributes(H)$score)
## R <- diag(solve(J%*%solve(I)%*%J))^0.5

## W <- weibreg(Surv(T,status)~X)
